...
test('babel-plugin-dynamic-import-node', (t) => {
  testFolders.forEach((folderName) => {
    ...
    versions.forEach((version) => {
      ...
      if (folderName !== 'template-argument') {
        // Babel 7 + preset-env + transform-template-literals currently fails with:
        // "Container is falsy" in babel traverse replaceWith method
      ...
      if (version === 6 && !process.env.OVERWRITE
        ...
        && folderName !== 'dynamic-argument' && folderName !== 'template-argument') {
