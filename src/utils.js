...
export function createDynamicImportTransform({ template, types: t }) {
  const builders = {
    static: {
      interop: template(`typeof require.ensure === 'function' ?
  (new Promise((resolve) => {require.ensure([], (require) => { resolve(require(SOURCE)); });})) :
  Promise.resolve().then(() => INTEROP(require(SOURCE)));`),
      noInterop: template(`typeof require.ensure === 'function' ?
  (new Promise((resolve) => {require.ensure([], (require) => { resolve(require(SOURCE)); });})) :
  Promise.resolve().then(() => require(SOURCE));`),
    ...
    dynamic: {
      interop: template(`typeof require.ensure === 'function' ?
  (new Promise((resolve) => {require.ensure([], (require) => { resolve(require(SOURCE)); });})) :
  Promise.resolve(SOURCE).then(s => INTEROP(require(s)));`),
      noInterop: template(`typeof require.ensure === 'function' ?
  (new Promise((resolve) => {require.ensure([], (require) => { resolve(require(SOURCE)); });})) :
  Promise.resolve(SOURCE).then(s => require(s));`),
    },
  };
